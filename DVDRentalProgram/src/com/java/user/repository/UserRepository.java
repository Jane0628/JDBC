...
public class UserRepository {
	...
	public void addUser(User user) {
		...
		/*
		 	...
		 	- try문에 자원 객체를 전달하면, try가 끝날 경우 자동으로 자원을 종료해주는 기능. (finally에서 따로 close() 안해도 됨)
		 	- 자원을 종료하려는 객체가 AutoCloseable이라는 인터페이스의 구현체여야 합니다.
		 */
		...
		try(Connection conn = connection.getConnection();
			...
			if(pstmt.executeUpdate() == 1) {
				...
				System.out.println("회원 가입에 실패하셨습니다. 관리자에게 문의하세요.");
			}
	...
	public List<User> findByUserName(String userName) {
		...
		try(Connection conn = connection.getConnection();
			...
			while(rs.next()) {
				User user = new User(rs.getInt("user_number"),
